...
void main() {
  ...
  patrolWidgetTest('Sign up with a taken address offers signing in', (t) async {
    await pumpApp(
      t,
  ...
  patrolWidgetTest('Sign up refused by the send throttle keeps the form open', (
    t,
  ) async {
    await pumpApp(
      t,
  ...
  patrolWidgetTest('Sign up failing on the server keeps the form open', (
    ...
    await pumpApp(t, serverStubbingSignUp(status: 500, message: 'Server down'));
    await startSignUp(t);
    expect(t('Server down'), findsOneWidget);
    expect(t(AppDebugKey.signUpScreen), findsOneWidget);
  });

  // Verify email: finishing the attempt.

  patrolWidgetTest('Verify the email address to finish signing up', (t) async {
  ...
  patrolWidgetTest('Send another code refused by the send throttle', (t) async {
    final server = serverStubbingSignUp()
      ..stubPost(
        '/signup/resend-verification',
        status: 429,
  ...
  patrolWidgetTest('Send another code again after the server fails', (t) async {
    var resends = 0;
    final server = serverStubbingSignUp()
      ..onPost(
        '/signup/resend-verification',
  ...
  patrolWidgetTest('Opening verification without an attempt goes to sign up', (
    t,
  ) async {
